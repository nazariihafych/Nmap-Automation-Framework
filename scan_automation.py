...
if not FERNET_KEY:
    ...
    logging.warning("FERNET_KEY не задан. Сгенерирован новый ключ. Сохрани его для повторного использования!")
...
async def start_scan():
    try:
        ...
        if scan_type not in SUPPORTED_SCAN_TYPES:
            return jsonify({"error": f"Недопустимый scan_type. Допустимые: {get_scan_type_choices()}"}), 400
...
async def periodic_scan(target: str, scan_type: str, interval_minutes: float):
    ...
    log_event(f"Запущено периодическое сканирование {target} каждые {interval_minutes} минут")
    await send_telegram_message(f"🔄 Запущено периодическое сканирование {target} каждые {interval_minutes} минут")
    while True:
        try:
